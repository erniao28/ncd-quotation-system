...
### 2026-03-08 - v1.4.0
**WebSocket 配置修复**
- 修复 Nginx WebSocket 代理配置，确保 `/socket.io` 路径正确转发到后端
- 创建独立的 Nginx 配置文件 `/etc/nginx/sites-available/ncd-quotation`
- 添加配置警告，防止其他项目修改此配置

**选中逻辑重构**
- 默认单击选中：清空之前的选中，只选中当前项
- Ctrl+ 单击/拖曳：累加选中（类似文件管理器行为）
- 添加独立的"取消全选"按钮（与"全选"分开）
- 拖曳选择逻辑优化：使用 ID 列表而非索引计算，解决排序后拖曳范围不正确的问题
