...
// AI 解析 API Key（只存在后端，不会泄露）
const API_KEY = 'AIzaSyC7NVK2pB9Ao9DtvntxHbWM5ZkpJYRzx1w';
...
// 解析到期日
router.post('/ai/parse-maturity', async (req, res) => {
  try {
    const { text } = req.body;
    ...
    console.error('[AI] 解析到期日失败:', error);
    res.status(500).json({ error: '解析失败' });
  }
});

// 解析报价
router.post('/ai/parse-quotation', async (req, res) => {
  try {
    const { text, defaultWeekday } = req.body;
    const ai = new GoogleGenAI({ apiKey: API_KEY });
    ...
    if (!resultText) return res.json([]);

    const result = JSON.parse(resultText);
    res.json(result);
  } catch (error) {
    console.error('[AI] 解析报价失败:', error);
    res.status(500).json({ error: '解析失败' });
  }
});
